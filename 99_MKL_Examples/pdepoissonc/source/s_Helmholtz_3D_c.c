...
/*
*  Content:
*  C real example of solving 3D Helmholtz problem in a
...
int main(void)
{
  ...
  s_Helmholtz_3D(f, bd_ax, bd_bx, bd_ay, bd_by, bd_az, bd_bz, &xhandle,
                                                         &yhandle, ipar, spar, &stat);
    if (stat!=0){
		error = 1;
		goto end;
...
/* Failure message to print if something went wrong */
	...
	if(error != 0)
	{
		printf("\nSingle precision 3D Helmholtz example FAILED to compute ");
		printf("the solution...\n");
		return 1;
	}
  /* Success message to print if everything is OK */
  printf("\n Single precision 3D Helmholtz example has successfully PASSED\n");
