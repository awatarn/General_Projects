...
/*
*  Content:
*  C double precision precision example of solving 2D Laplace problem in a
...
int main(void)
{
  ...
  MKL_INT ipar[128];
  double ax, bx, ay, by, lx, ly, hx, hy, xi, yi, c1;
  double *dpar=NULL, *f=NULL, *u=NULL, *bd_ax=NULL, *bd_bx=NULL, *bd_ay=NULL, *bd_by=NULL;
  double q;
  ...
  f=(double*)mkl_malloc((nx+1)*(ny+1)*sizeof(double), 64);
  if(f == NULL) goto end;
  u=(double*)mkl_malloc((nx+1)*(ny+1)*sizeof(double), 64);
  if(u == NULL) goto end;
  ...
  *******************************************************************************
  ...
  /* Failure message to print if something went wrong */
	...
	if(error != 0)
	{
		 printf("\nDouble precision 2D Laplace example FAILED to compute ");
         printf("the solution...\n");
         return 1;
	}
  /* Success message to print if everything is OK */
  printf("\n Double precision 2D Laplace example has successfully PASSED\n");
