...
*  excluding poles using Intel(R) MKL Poisson Library
*
*******************************************************************************/
...
int main(void)
{
  ...
  /* Computing the approximate solution of Helmholtz problem on a sphere
  without poles */
...
/* Failure message to print if something went wrong */
	...
	if(error != 0)
	{
		 printf("\nDouble precision Helmholtz example on a sphere without poles ");
         printf("has FAILED to compute the solution...\n");
         return 1;
	}
  /* Success message to print if everything is OK */
  printf("\n Double precision Helmholtz example on a sphere without poles has ");
