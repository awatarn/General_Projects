...
/*
*  Content:
*  C real example of solving 3D Poisson problem in a
*  parallelepiped domain using Intel(R) MKL Poisson Library
*
*******************************************************************************/
...
int main(void)
{
  ...
  MKL_INT ipar[128];
  float ax, bx, ay, by, az, bz, lx, ly, lz, hx, hy, hz, xi, yi, zi, cx, cy, cz, c1;
  float *spar=NULL, *f=NULL, *u=NULL, *bd_ax=NULL, *bd_bx=NULL, *bd_ay=NULL, *bd_by=NULL, *bd_az=NULL, *bd_bz=NULL;
  float q;
  ...
  /* memory allocation */
  mem_error = 1;
  spar=(float*)mkl_malloc((13*(nx+ny)/2+9)*sizeof(float), 64);
  if(spar == NULL) goto end;
  f=(float*)mkl_malloc((nx+1)*(ny+1)*(nz+1)*sizeof(float), 64);
  if(f == NULL) goto end;
  u=(float*)mkl_malloc((nx+1)*(ny+1)*(nz+1)*sizeof(float), 64);
  if(u == NULL) goto end;
  bd_ax=(float*)mkl_malloc((ny+1)*(nz+1)*sizeof(float), 64);
  if(bd_ax == NULL) goto end;
  bd_bx=(float*)mkl_malloc((ny+1)*(nz+1)*sizeof(float), 64);
  if(bd_bx == NULL) goto end;
  bd_ay=(float*)mkl_malloc((nx+1)*(nz+1)*sizeof(float), 64);
  if(bd_ay == NULL) goto end;
  bd_by=(float*)mkl_malloc((nx+1)*(nz+1)*sizeof(float), 64);
  if(bd_by == NULL) goto end;
  bd_az=(float*)mkl_malloc((nx+1)*(ny+1)*sizeof(float), 64);
  if(bd_az == NULL) goto end;
  bd_bz=(float*)mkl_malloc((nx+1)*(ny+1)*sizeof(float), 64);
  ...
  *******************************************************************************/
  ...
  for(iz=0;iz<=nz;iz++)
  {
    for(iy=0;iy<=ny;iy++)
    {
      for(ix=0;ix<=nx;ix++)
      {
        ...
        f[ix+iy*(nx+1)+iz*(nx+1)*(ny+1)]=
                                      (12.0E0*pi*pi)*u[ix+iy*(nx+1)+iz*(nx+1)*(ny+1)];
  ...
  s_init_Helmholtz_3D(&ax, &bx, &ay, &by, &az, &bz, &nx, &ny, &nz, BCtype, &q,
                                                                  ipar, spar, &stat);
    if (stat!=0){ 
		error = 1;
		goto end;
  ...
  s_commit_Helmholtz_3D(f, bd_ax, bd_bx, bd_ay, bd_by, bd_az, bd_bz, &xhandle,
  ...
  NOTE: Boundary data stored in the arrays bd_ax, bd_bx, bd_ay, bd_by, bd_az,
  bd_bz should not be changed between the Commit step and the subsequent call to
  the Solver routine! Otherwise, the results may be wrong. */
  s_Helmholtz_3D(f, bd_ax, bd_bx, bd_ay, bd_by, bd_az, bd_bz, &xhandle, &yhandle,
                                                                  ipar, spar, &stat);
    if (stat!=0){
		error = 1;
		goto end;
...
/* Failure message to print if something went wrong */
	...
	if(error != 0)
	{
		printf("\nSingle precision 3D Poisson example FAILED to compute ");
		printf("the solution...\n");
		return 1;
	}
  /* Success message to print if everything is OK */
  printf("\n Single precision 3D Poisson example has successfully PASSED\n");
