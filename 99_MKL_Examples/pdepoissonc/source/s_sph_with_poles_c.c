...
int main(void)
{
  ...
  for(it=0;it<=nt;it++)
  {
    printf("In the mesh point (%5.3f,%5.3f) the error between ", ip*hp, it*ht);
    printf("the computed and the true solution is equal to ");
...
/* Failure message to print if something went wrong */
	...
	if(error != 0)
	{
		 printf("\nSingle precision Helmholtz example on a whole sphere has ");
