...
#define A31 0.0, -1.0, \
            0.0,  0.0,
    // ******************************************************************************
    //    Sparse representation of the matrix A in BSR format
    // ******************************************************************************
/*    double bsrVal[BSR_NNZ]    = { 4.0, -1.0,       -1.0,
                                 -1.0,  4.0, -1.0,       -1.0,
                                       -1.0,  4.0,             -1.0,
                                 -1.0,              4.0, -1.0,
                                       -1.0,       -1.0,  4.0, -1.0,
                                             -1.0,       -1.0,  4.0 }; */
    double bsrVal[BSR_NNZ] = { A11 A12 A13
                               A12 B11 A12
                               A31 A12 A11 };
    MKL_INT bsrColInd[BSR_NNZ] = { 0, 1, 2, 0, 1, 2, 0, 1, 2 };
    MKL_INT bsrRowPtr[BSR_M+1] = { 0, 3, 6, 9 };
    ...
    mkl_sparse_d_create_bsr ( &bsrA, SPARSE_INDEX_BASE_ZERO, SPARSE_LAYOUT_ROW_MAJOR, BSR_M, BSR_M, BSR_BLOCKSIZE, bsrRowPtr, bsrRowPtr+1, bsrColInd, bsrVal );
...
//    mkl_sparse_set_mv_hint  ( bsrA, SPARSE_OPERATION_NON_TRANSPOSE, descrA_matrix, expected_calls );

    // Create matrix descriptors for matrix and preconditioner
    descrA_precond.mode = SPARSE_FILL_MODE_LOWER;
    descrA_precond.diag = SPARSE_DIAG_NON_UNIT;
    descrA_precond.type = SPARSE_MATRIX_TYPE_BLOCK_TRIANGULAR;
    mkl_sparse_set_sv_hint  ( bsrA, SPARSE_OPERATION_NON_TRANSPOSE, descrA_precond, expected_calls);

    descrA_precond.type = SPARSE_MATRIX_TYPE_BLOCK_DIAGONAL;
    mkl_sparse_set_mv_hint  ( bsrA, SPARSE_OPERATION_NON_TRANSPOSE, descrA_precond, expected_calls);
    ...
    // Calculation B^{-1}r_0
    {
       descrA_precond.type = SPARSE_MATRIX_TYPE_BLOCK_TRIANGULAR;
       descrA_precond.mode = SPARSE_FILL_MODE_LOWER;
       mkl_sparse_d_trsv ( SPARSE_OPERATION_NON_TRANSPOSE, 1.0, bsrA, descrA_precond, r, tmp);

       descrA_precond.type = SPARSE_MATRIX_TYPE_BLOCK_DIAGONAL;
       mkl_sparse_d_mv ( SPARSE_OPERATION_NON_TRANSPOSE, 1.0, bsrA, descrA_precond, tmp, zero, tmp2 );

       descrA_precond.type = SPARSE_MATRIX_TYPE_BLOCK_TRIANGULAR;
       descrA_precond.mode = SPARSE_FILL_MODE_UPPER;
       mkl_sparse_d_trsv ( SPARSE_OPERATION_NON_TRANSPOSE, 1.0, bsrA, descrA_precond, tmp2, w);
    }
    ...
    {
        // Calculate A*p
        mkl_sparse_d_mv ( SPARSE_OPERATION_NON_TRANSPOSE, 1.0, bsrA, descrA_matrix, p, zero, tmp );
        // Calculate alpha_k
        temp2 = 0.0;
        ...
        mkl_sparse_d_mv ( SPARSE_OPERATION_NON_TRANSPOSE, -alpha, bsrA, descrA_matrix, p, one, r );
