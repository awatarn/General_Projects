...
!          MKL_SCSCSM  MKL_SCSCSV  MKL_SCSCMM  MKL_SCSCMV.
!
...
!
!       1. The code computes (L+D)'*S = F using MKL_SCSCMM where S is a known 5 by 2
!          matrix and then the code solves the system (L+D)'*X = F with the help of MKL_SCSCSM.
!          It's evident that X should be equal to S.
!
!       2. The code computes (U+I)'*S = F using MKL_SCSCMV where S is a vector
!          and then the code calls MKL_SCSCTRSV solves the system (U+I)'*X = F with the single right
!          hand side. It's evident that X should be equal to S.
!
!       3. The code computes D*S = F using MKL_SCSCMV where S is a vector
!          and then the code solves the system D*X = F with the single right hand side.
!          It's evident that X should be equal to S.
!
!       4. The next step is the computation (U-U') S = F using MKL_SCSCMV where S is
!          a vector. It is easy to see that U-U' is a skew-symmetric matrix.
!
!       5. The next step is the computation (L+D+L') S = F using MKL_SCSCSYMV where S is
!          a vector. It is easy to see that L+D+L' is a symmetric matrix.
!
!       6. The next step is the computation A'* S = F using MKL_SCSCGEMV where S is
!          a vector.
!
...
!          T*X =F using MKL_SCSCSV. The routine MKL_SCSCMV is used for getting matrix-vector multiply.
!
...
#define MNEW 3
    ...
    float       values[NNZ]   = {1.0, -2.0, -4.0, -1.0, 5.0, 8.0, 4.0, 2.0, -3.0, 6.0, 7.0, 4.0, -5.0};
...
//*******************************************************************************
    ...
    mkl_scscmm(&transa, &m, &n, &m, &alpha, matdescra, values, rows, colIndex, &(colIndex[1]), &(sol[0][0]), &n,  &beta, &(rhs[0][0]), &n);
    ...
    mkl_scscsm(&transa, &m, &n, &alpha, matdescra, values, rows, colIndex, &(colIndex[1]), &(rhs[0][0]), &n, &(temp[0][0]), &n);

    printf("                             \n");
    printf("   OUTPUT DATA FOR MKL_SCSCSM\n");
    for (i = 0; i < m; i++) {
        for (j = 0; j < n; j++) {
...
//*******************************************************************************
    ...
    printf("                             \n");
    printf("   INPUT DATA FOR MKL_SCSCMV \n");
    printf("   WITH TRIANGULAR MATRIX    \n");
    printf("     ALPHA = %4.1f  BETA = %4.1f \n", alpha, beta);
    printf("     TRANS = '%c' \n", 'T');
    printf("   Input vector              \n");
    ...
    mkl_scscmv(&transa, &m, &m, &alpha, matdescra, values, rows, pointerB, pointerE, sol_vec, &beta, rhs_vec);

    printf("                             \n");
    printf("   OUTPUT DATA FOR MKL_SCSCMV\n");
    printf("   WITH TRIANGULAR MATRIX    \n");
    for (i = 0; i < m; i++) {
        printf("%7.1f\n", rhs_vec[i]);
    };
    ...
    mkl_scscsv(&transa, &m, &alpha, matdescra, values, rows, colIndex, &(colIndex[1]),  rhs_vec, temp_vec);
...
//*******************************************************************************
    ...
    printf("   of MKL_SCSCSV            \n");


    mkl_scscsv(&transa, &m, &alpha, matdescra, values, rows, colIndex, &(colIndex[1]), rhs_vec, temp_vec);

    printf("                             \n");
    printf("   OUTPUT DATA FOR MKL_SCSCSV\n");
    printf("   WITH DIAGONAL MATRIX      \n");
    for (i = 0; i < m; i++) {
        printf("%7.1f\n", temp_vec[i]);
    };
...
//*******************************************************************************
    ...
    matdescra[2] = 'n';
    matdescra[3] = 'c';
    mkl_scscmv(&transa, &m, &m, &alpha, matdescra, values, rows, colIndex, &(colIndex[1]), sol_vec, &beta, rhs_vec);
    printf("                             \n");
...
//*******************************************************************************
    ...
    printf("-----------------------------------------------\n");
//*******************************************************************************
// Task 6.   Obtain matrix-vector multiply A'*sol --> rhs with the help of MKL_SCSCMV
//
//*******************************************************************************
    printf("                             \n");
    printf("   INPUT DATA FOR            \n");
...
//*******************************************************************************
// Task 7.  Obtain matrix-vector multiply T*sol --> rhs with the help of MKL_SCSCMV
...
//*******************************************************************************
    ...
    mkl_scscmv(&transa, &mnew, &mnew, &alpha, matdescra, values, rows, pointerB, pointerE, sol_vec, &beta, rhs_vec);
    ...
    mkl_scscsv(&transa, &mnew, &alpha, matdescra, values, rows, pointerB, pointerE, rhs_vec, temp_vec);
