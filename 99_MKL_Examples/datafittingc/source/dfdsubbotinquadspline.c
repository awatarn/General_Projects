...
int main()
{
    ...
    double ic[NIC];                     // array of Subbotin spline knots
    double y[N*NY];                     // function values
    double bc[NBC] = { 1.0, -1.0 };     // array of boundary conditions
    double scoeff[NSCOEFF];             // array of spline coefficients

    double left = LEFT_LIMIT, right = RIGHT_LIMIT;
    double freq = FREQ;
    double spline_val[N], left_val[N], right_val[N];
    double left_der[N], right_der[N];
    ...
    errcode = dCheckSubbBreakPoints( nx, x, ny, y, scoeff,
                                     spline_val );
    if ( errcode < 0 ) errnums++;

    /***** Check spline values and 1st derivatives consistency
           in Subbotin spline knots *****/
    errcode = dCheckSubbQNodes( nx, x, nic, ic, ny, scoeff,
                                left_val, right_val );
    if ( errcode < 0 ) errnums++;

    errcode = dCheckQuadSubb1stDerConsistency( nic, x, ic, ny, scoeff,
                                               left_der, right_der );
    if ( errcode < 0 ) errnums++;
