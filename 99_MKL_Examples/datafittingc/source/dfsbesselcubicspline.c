...
!
! API
!   int  linear_extrap( MKL_INT64* n, MKL_INT64 cell[], float site[],
!                       float r[], void *user_params,
...
!   >0 - warning
*******************************************************************************/
int  linear_extrap( MKL_INT64* n, MKL_INT64 cell[], float site[], float r[],
                    void *user_params,
                    dfInterpCallBackLibraryParams* library_params );
...
int main()
{
    ...
    float lsite = LLIM_SITE;              // left  limit of interpolation sites
    float rsite = RLIM_SITE;              // right limit of interpolation sites
    float x[N];                           // break points
    float y[N*NY];                        // function values
    float *bc;                            // boundary condition
    float *ic;                            // internal condition
    float scoeff[NSCOEFF];                // Bessel spline coefficients
    ...
    errcode = dfsNewTask1D( &task, nx, x, xhint, ny, y, DF_NO_HINT );
    CheckDfError(errcode);

    /***** Edit task parameters for Bessel cubic spline coefficients
           computation *****/
    errcode = dfsEditPPSpline1D( task, sorder, stype, bc_type, 0, ic_type, 0,
                                 scoeff, scoeffhint );
    CheckDfError(errcode);
    ...
    /***** Interpolate and use call backs for the left and right
           extrapolation *****/
    errcode = dfsInterpolateEx1D( task, DF_INTERP, DF_METHOD_PP, nsite, site,
    ...
    errcode = sCheckCubInterpRes( nx, x, ny, scoeff,
    ...
    for ( i = 0; i < nsite; i++ )
    {
        if ( site[i] < x[0] )
        {
            ...
            sFindCells( nx, x, (MKL_INT)1, &site[i], (MKL_INT*)(&tmp_cell) );

            errcode = linear_extrap( &n1, &tmp_cell, &site[i], &ref_r[i],
                &le_params, NULL );

            if ( DF_ABS(ref_r[i] - r[i]) > EPSILON_SINGLE )
                errnums++;
        }
        if ( site[i] > x[nx-1] )
        {
