...
int main()
{
    ...
    MKL_INT *cell_idx;          // indices of cells containing
                                // interpolation sites
    float  y[N*NY]={10,10,10,10,10,10,10.5,15,50,60,85}; // function values
    float  x[N   ]={0 ,2 ,3 ,5 ,6 ,8 ,9   ,11,12,14,15}; // breakpoints
    float  *ic;                 // array of internal conditions
    float  bc[2];               // array of boundary conditions
    float  scoeff[NSCOEFF];     // array of spline coefficients
    float  site[NSITE];         // array of interpolation sites
    float  r[NDER*NSITE*NY];    // spline evaluation results
    float  *datahint;           // additional info about structure
                                // of arrays x and y
    int xi,xxi,errcode;
    int comonot_status,comonot_status_1interval;
    float  slope,di,dip1,alpha,beta;
    ...
    errcode = dfsNewTask1D(&interpTask, nx,x,xhint, ny,y,yhint);
    CheckDfError(errcode);

    /***** Edit task parameters for natural cubic spline construction *****/
    errcode = dfsEditPPSpline1D(interpTask, sorder, stype, bc_type, bc,
        ic_type, ic, scoeff, scoeffhint);
    CheckDfError(errcode);

    /***** Construct co-monotone cubic spline *****/
    errcode = dfsConstruct1D(interpTask, DF_PP_SPLINE, DF_METHOD_STD);
    CheckDfError(errcode);
    ...
    */
    ...
    for(xi=0;xi<=nx-2;xi++) /* For each sub-interval */
    {
        ...
        for(xxi=0;xxi<nsite;xxi++) {
            site[xxi] = x[xi] + (x[xi+1]-x[xi])/(float)(NSITE-1)*(float)xxi;
        }
        errcode = dfsInterpolate1D(interpTask, DF_INTERP, DF_METHOD_PP,
