...
extern void print_eigenvalues( char* desc, int n, float* wr, float* wi );
extern void print_eigenvectors( char* desc, int n, float* wi, float* v,
      int ldv );
...
int main() {
	...
	lwork = -1;
	sgeev( "Vectors", "Vectors", &n, a, &lda, wr, wi, vl, &ldvl, vr, &ldvr,
         &wkopt, &lwork, &info );
	lwork = (MKL_INT)wkopt;
