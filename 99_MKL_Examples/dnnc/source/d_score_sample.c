...
static dnnError_t simple_net(int want_groups_conv) {
    ...
    /*** LRN section ***/
    CHECK_ERR( dnnLRNCreateForward_F64(&lrn1, attributes, lt_relu1_output, 5, 1.0f, 0.75f, 0.75f), err );

    resLrn1[dnnResourceSrc] = resRelu1[dnnResourceDst];

    CHECK_ERR( dnnLayoutCreateFromPrimitive_F64(&lt_lrn1_output, lrn1, dnnResourceDst), err );
    CHECK_ERR( dnnLayoutCreateFromPrimitive_F64(&lt_lrn1_workspace, lrn1, dnnResourceWorkspace), err );
    CHECK_ERR( dnnAllocateBuffer_F64(&resLrn1[dnnResourceDst], lt_relu1_output) , err );
    CHECK_ERR( dnnAllocateBuffer_F64(&resLrn1[dnnResourceWorkspace], lt_lrn1_workspace) , err );
    ...
    for (k = 0; k < 1; ++k) {
        ...
        CHECK_ERR( dnnExecute_F64(lrn1,  (void*)resLrn1) , err );
        CHECK_ERR( dnnExecute_F64(pool1, (void*)resPool1), err );

        if (cv_pool1_to_user_output) CHECK_ERR( dnnConversionExecute_F64(cv_pool1_to_user_output, resPool1[dnnResourceDst], user_o), err );
        // Shift pointers
    }
...
bail_out:
    ...
    dnnLayoutDelete_F64(lt_lrn1_output);
    dnnLayoutDelete_F64(lt_lrn1_workspace);
    ...
    dnnReleaseBuffer_F64(resLrn1[dnnResourceDst]);
    dnnReleaseBuffer_F64(resLrn1[dnnResourceWorkspace]);
    dnnReleaseBuffer_F64(resPool1[dnnResourceDst]);
    dnnReleaseBuffer_F64(resPool1[dnnResourceWorkspace]);
    if ((void*)user_o != resPool1[dnnResourceDst]) dnnReleaseBuffer_F64((void*)user_o);
