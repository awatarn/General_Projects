...
static dnnError_t simple_net() {
    ...
    float* resPool1[dnnResourceNumber] = {0};
    ...
    CHECK_ERR( dnnLayoutCreate_F32(&lt_user_output, dimension,
    ...
    CHECK_ERR( dnnConvolutionCreateBackwardData_F32 (&conv_bwd_data, attributes,
                dnnAlgorithmConvolutionDirect, dimension, inputSize,
                outputSize, filterSize, convolutionStride, inputOffset,
                dnnBorderZeros), err);


    CHECK_ERR( dnnLayoutCreateFromPrimitive_F32(&lt_conv_diff_src, conv_bwd_data,
               dnnResourceDiffSrc), err );
    CHECK_ERR( dnnAllocateBuffer_F32((void**)&resConv1[dnnResourceDiffSrc],
                lt_conv_diff_src), err );

    /*  Convolution backward filter  */
    CHECK_ERR( dnnConvolutionCreateBackwardFilter_F32 (&conv_bwd_filter, attributes,
                dnnAlgorithmConvolutionDirect, dimension, inputSize,
                outputSize, filterSize, convolutionStride, inputOffset,
                dnnBorderZeros), err);

    CHECK_ERR( dnnLayoutCreateFromPrimitive_F32(&lt_conv_diff_filter, conv_bwd_filter,
                dnnResourceDiffFilter), err );
    CHECK_ERR( dnnAllocateBuffer_F32((void**)&resConv1[dnnResourceDiffFilter],
                lt_conv_diff_filter), err );

    /*  Convolution backward bias  */
    CHECK_ERR( dnnConvolutionCreateBackwardBias_F32 (&conv_bwd_bias, attributes,
               dnnAlgorithmConvolutionDirect, dimension, outputSize), err);

    CHECK_ERR( dnnLayoutCreateFromPrimitive_F32(&lt_conv_diff_bias, conv_bwd_bias,
                dnnResourceDiffBias), err );
    CHECK_ERR( dnnAllocateBuffer_F32((void**)&resConv1[dnnResourceDiffBias],
    ...
    CHECK_ERR( dnnAllocateBuffer_F32((void**)&resPool1[dnnResourceDst], lt_pool1_output) , err );
    CHECK_ERR( dnnAllocateBuffer_F32((void**)&resPool1[dnnResourceWorkspace], lt_pool1_workspace) , err );
    ...
    /* Pooling backward */
    CHECK_ERR( dnnPoolingCreateBackward_F32(&pool_bwd, attributes, dnnAlgorithmPoolingMax,
                lt_pool1_input, kernelSize, kernelStride, inputOffset,
                dnnBorderZeros), err );

    CHECK_ERR( dnnLayoutCreateFromPrimitive_F32(&lt_pool_diff, pool_bwd,
                dnnResourceDiffSrc), err );
    CHECK_ERR( dnnAllocateBuffer_F32((void**)&resPool1[dnnResourceDiffSrc],
                lt_pool_diff), err );
    ...
    for (k = 0; k < 1; ++k) {
        ...
        if (cv_pool_to_user_output) CHECK_ERR( dnnConversionExecute_F32(cv_pool_to_user_output, resPool1[dnnResourceDst], user_o), err );
        // Calculation of output diff and storing results into user_o
        if (cv_user_to_pool_output) CHECK_ERR( dnnConversionExecute_F32(cv_user_to_pool_output, user_o, resPool1[dnnResourceDiffDst]), err );

        CHECK_ERR( dnnExecute_F32(pool_bwd, (void*)resPool1), err );
        CHECK_ERR( dnnExecute_F32(conv_bwd_data, (void*)resConv1), err );
        CHECK_ERR( dnnExecute_F32(conv_bwd_filter, (void*)resConv1), err );
        CHECK_ERR( dnnExecute_F32(conv_bwd_bias, (void*)resConv1), err );
...
bail_out:
    ...
    dnnDelete_F32(conv1);
    dnnDelete_F32(conv_bwd_data);
    dnnDelete_F32(conv_bwd_filter);
    dnnDelete_F32(conv_bwd_bias);
    dnnDelete_F32(pool1);
    dnnDelete_F32(pool_bwd);
    dnnDelete_F32(cv_user_to_conv1_input);
    dnnDelete_F32(cv_user_to_conv1_filt);
    dnnDelete_F32(cv_user_to_conv1_bias);
    dnnDelete_F32(cv_pool_to_user_output);
    dnnDelete_F32(cv_user_to_pool_output);
    ...
    dnnReleaseBuffer_F32(resConv1[dnnResourceDiffSrc]);
    dnnReleaseBuffer_F32(resConv1[dnnResourceDiffFilter]);
    dnnReleaseBuffer_F32(resConv1[dnnResourceDiffBias]);
    dnnReleaseBuffer_F32(resPool1[dnnResourceDst]);
    dnnReleaseBuffer_F32(resPool1[dnnResourceWorkspace]);
    dnnReleaseBuffer_F32(resPool1[dnnResourceDiffSrc]);
    if ((void*)user_o != resPool1[dnnResourceDst]) dnnReleaseBuffer_F32(user_o);
    if (resPool1[dnnResourceDiffDst] != (void*)user_o) dnnReleaseBuffer_F32(resPool1[dnnResourceDiffDst]);
