...
/*
!  Content:
!      C B L A S _ S G E M M _ C O M P U T E  Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
      ...
      printf("\n     C B L A S _ S G E M M _ C O M P U T E  EXAMPLE PROGRAM\n");
...
/*      Call SGEMM_ALLOC function to allocate buffer for A (C Interface) */
      identifier = CblasAMatrix;
      dest = cblas_sgemm_alloc(identifier, m, n, k);
      if( dest == NULL ) {
...
/*      Call SGEMM_PACK subroutine to perform scaling and packing (C Interface) */
      cblas_sgemm_pack(layout, identifier, transA, m, n, k, alpha, a, lda, dest);
/*      Call SGEMM_COMPUTE subroutine (C Interface)                     */
      storage = CblasPacked;
      cblas_sgemm_compute(layout, storage, transB, m, n, k,
                  dest, lda, b, ldb, beta, c, ldc);
/*      Call SGEMM_FREE subroutine to free allocated storage (C Interface) */
      cblas_sgemm_free(dest);
