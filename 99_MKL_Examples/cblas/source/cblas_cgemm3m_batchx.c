...
/*
!  Content:
!      C B L A S _ C G E M M 3 M _ B A T C H  Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
      ...
      MKL_Complex8    alpha[MAX_GRP_COUNT], beta[MAX_GRP_COUNT];
      MKL_Complex8   *a, *b, *c;
      MKL_Complex8  **a_array, **b_array, **c_array;
      CBLAS_LAYOUT    layout;
      CBLAS_TRANSPOSE transA[MAX_GRP_COUNT], transB[MAX_GRP_COUNT];
      MKL_INT         ma, na, mb, nb;
      MKL_INT         grp_count, grp_sizes[MAX_GRP_COUNT], total_batch_count, idx;

      printf("\n     C B L A S _ C G E M M 3 M _ B A T C H  EXAMPLE PROGRAM\n");
...
/*       Get input data                                        */
      ...
      for (i=0; i<grp_count; ++i) {
          if( GetScalarsC(in_file, &alpha[i], &beta[i]) != 2 ) {
      ...
      a_array = (MKL_Complex8 **)mkl_calloc(total_batch_count, sizeof( MKL_Complex8* ), 64);
      b_array = (MKL_Complex8 **)mkl_calloc(total_batch_count, sizeof( MKL_Complex8* ), 64);
      c_array = (MKL_Complex8 **)mkl_calloc(total_batch_count, sizeof( MKL_Complex8* ), 64);
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              ...
              if( GetArrayC(in_file, &layout, GENERAL_MATRIX, &ma, &na, a, &lda[i]) != 0 ) {
                  printf("\n ERROR of array A reading, group idx="INT_FORMAT" matrix idx="INT_FORMAT, i, idx);
                  printf("\n");
                  return 1;
              }
              if( GetArrayC(in_file, &layout, GENERAL_MATRIX, &mb, &nb, b, &ldb[i]) != 0 ) {
                  printf("\n ERROR of array B reading, group idx="INT_FORMAT" matrix idx="INT_FORMAT, i, idx);
                  printf("\n");
                  return 1;
              }
              if( GetArrayC(in_file, &layout, GENERAL_MATRIX, &m[i],  &n[i],  c, &ldc[i]) != 0 ) {
...
/*       Print input data                                      */
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              PrintArrayC(&layout, FULLPRINT, GENERAL_MATRIX, &ma, &na, a_array[idx], &lda[i], "A");
              PrintArrayC(&layout, FULLPRINT, GENERAL_MATRIX, &mb, &nb, b_array[idx], &ldb[i], "B");
              PrintArrayC(&layout, FULLPRINT, GENERAL_MATRIX, &m[i],  &n[i],  c_array[idx], &ldc[i], "C");
              idx++;
          }
      }

/*      Call CGEMM3M_BATCH subroutine ( C Interface )                  */
      cblas_cgemm3m_batch(layout, &transA[0], &transB[0], &m[0], &n[0], &k[0], &alpha[0],
              a_array, &lda[0], b_array, &ldb[0], &beta[0], c_array, &ldc[0], grp_count, &grp_sizes[0]);
