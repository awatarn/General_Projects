...
/*
!  Content:
!      C B L A S _ D G E M M _ B A T C H Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
      ...
      double          alpha[MAX_GRP_COUNT], beta[MAX_GRP_COUNT];
      double         *a, *b, *c;
      ...
      printf("\n     C B L A S _ D G E M M _ B A T C H  EXAMPLE PROGRAM\n");
...
/*       Get input data                                        */
      ...
      for (i=0; i<grp_count; ++i) {
          if( GetScalarsD(in_file, &alpha[i], &beta[i]) != 2 ) {
      ...
      c_array = (double **)mkl_calloc(total_batch_count, sizeof( double* ), 64);
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              ...
              if( GetArrayD(in_file, &layout, GENERAL_MATRIX, &ma, &na, a, &lda[i]) != 0 ) {
                  printf("\n ERROR of array A reading, group idx="INT_FORMAT" matrix idx="INT_FORMAT, i, idx);
                  printf("\n");
                  return 1;
              }
              if( GetArrayD(in_file, &layout, GENERAL_MATRIX, &mb, &nb, b, &ldb[i]) != 0 ) {
                  printf("\n ERROR of array B reading, group idx="INT_FORMAT" matrix idx="INT_FORMAT, i, idx);
                  printf("\n");
                  return 1;
              }
              if( GetArrayD(in_file, &layout, GENERAL_MATRIX, &m[i],  &n[i],  c, &ldc[i]) != 0 ) {
...
/*       Print input data                                      */
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              PrintArrayD(&layout, FULLPRINT, GENERAL_MATRIX, &ma, &na, a_array[idx], &lda[i], "A");
              PrintArrayD(&layout, FULLPRINT, GENERAL_MATRIX, &mb, &nb, b_array[idx], &ldb[i], "B");
              PrintArrayD(&layout, FULLPRINT, GENERAL_MATRIX, &m[i],  &n[i],  c_array[idx], &ldc[i], "C");
              idx++;
          }
      }

/*      Call DGEMM_BATCH subroutine ( C Interface )                  */
      cblas_dgemm_batch(layout, transA, transB, m, n, k, alpha,
