...
/*
! Content:
!       Example of using fftw_plan_guru_split_dft function.
!
!****************************************************************************/
...
static void init_i(double *re, double *im,
                   fftw_iodim *ndims, fftw_iodim *vdims,
...
int main(void)
{
    ...
    double *x_re = 0, *x_im = 0, *y_re = 0, *y_im = 0;

    /* Execution status */
    int status = 0;


    printf("Example dp_plan_guru_split_dft\n");
    ...
    y_re  = fftw_malloc(sizeof(double) * osize2(3,dim,1,&M_dim) );
    y_im  = fftw_malloc(sizeof(double) * osize2(3,dim,1,&M_dim) );
    if (0 == y_re || 0 == y_im) goto failed;

    printf("Create FFTW plan for forward transform\n");
    fwd = fftw_plan_guru_split_dft(3, dim, 1, &M_dim, x_re, x_im, y_re, y_im,
                                   FFTW_ESTIMATE);
    if (0 == fwd) goto failed;

    printf("Create FFTW plan for backward transform\n");
    swapio2(3,dim,1,&M_dim);
    bwd = fftw_plan_guru_split_dft(3, dim, 1, &M_dim, y_im, y_re, x_im, x_re,
                                   FFTW_ESTIMATE);
    ...
    fftw_execute_split_dft(bwd, y_im, y_re, x_im, x_re);
