...
/*
! Content:
!       Example of using fftwf_plan_guru64_dft function.
!
!****************************************************************************/
...
static void init_i(fftwf_complex *x, fftwf_iodim64 *ndims, fftwf_iodim64 *vdims,
                   ptrdiff_t H1, ptrdiff_t H2, ptrdiff_t H3);
static int verify_o(fftwf_complex *x, fftwf_iodim64 *ndims, fftwf_iodim64 *vdims,
                    ptrdiff_t H1, ptrdiff_t H2, ptrdiff_t H3);
static ptrdiff_t isize2(int n, fftwf_iodim64 *ndims, int m, fftwf_iodim64 *vdims);
static ptrdiff_t osize2(int n, fftwf_iodim64 *ndims, int m, fftwf_iodim64 *vdims);
static void swapio2(int n, fftwf_iodim64 *ndims, int m, fftwf_iodim64 *vdims);
...
int main(void)
{
    ...
    fftwf_complex *x = 0, *y = 0;

    /* Execution status */
    int status = 0;


    printf("Example sp_plan_guru64_dft\n");
    ...
    x  = fftwf_malloc(sizeof(fftwf_complex) * isize2(3,dim,1,&M_dim) );
    if (0 == x) goto failed;

    printf("Allocate y("LI")\n", osize2(3,dim,1,&M_dim) );
    y  = fftwf_malloc(sizeof(fftwf_complex) * osize2(3,dim,1,&M_dim) );
    if (0 == y) goto failed;

    printf("Create FFTW plan for forward transform\n");
    fwd = fftwf_plan_guru64_dft(3, dim, 1, &M_dim, x, y, FFTW_FORWARD,
                             FFTW_ESTIMATE);
    if (0 == fwd) goto failed;

    printf("Create FFTW plan for backward transform\n");
    swapio2(3,dim,1,&M_dim);
    bwd = fftwf_plan_guru64_dft(3, dim, 1, &M_dim, y, x, FFTW_BACKWARD,
                             FFTW_ESTIMATE);
