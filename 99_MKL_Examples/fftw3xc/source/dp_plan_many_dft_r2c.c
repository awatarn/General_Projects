...
/*
! Content:
!       Example of using fftw_plan_many_dft_r2c function.
!
!****************************************************************************/
...
static void init_r(double *x,
                   int *N, int M, int *EN, int stride, int dist, int *H);
static int verify_c(fftw_complex *x,
                    int *N, int M, int *EN, int stride, int dist, int *H);
...
int main(void)
{
    ...
    printf("Example dp_plan_many_dft_r2c\n");
    ...
    x  = fftw_malloc(sizeof(double) * EM*dist );
    if (0 == x) goto failed;
    ...
    printf("Create FFTW plan for r2c transform\n");
    {
        ...
        ENc[1] = EN[1];
        ENc[2] = EN[2] / 2;
        r2c = fftw_plan_many_dft_r2c(3, N, M,
                                     x, EN, stride, dist,
                                     (fftw_complex*)x, ENc, stride, dist/2,
                                     FFTW_ESTIMATE);
    ...
    status = verify_c((fftw_complex*)x, N, M, EN, stride, dist/2, H);
