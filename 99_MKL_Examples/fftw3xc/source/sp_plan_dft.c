...
/*
! Content:
!       Example of using fftwf_plan_dft function.
!
!****************************************************************************/
...
int main(void)
{
    ...
    printf("Example sp_plan_dft\n");
    printf("Forward and backward 4D complex inplace transform\n");
    printf("Configuration parameters:\n");
    printf(" N = {%d, %d, %d, %d}\n", N[0], N[1], N[2], N[3]);
    printf(" H = {%d, %d, %d, %d}\n", H[0], H[1], H[2], H[3]);

    printf("Allocate array for input data\n");
    x  = fftwf_malloc(sizeof(fftwf_complex)*N[0]*N[1]*N[2]*N[3]);
    if (0 == x) goto failed;

    printf("Create FFTW plan for 4D double-precision forward transform\n");
    forward_plan = fftwf_plan_dft(4, N, x, x, FFTW_FORWARD, FFTW_ESTIMATE);
    if (0 == forward_plan) goto failed;

    printf("Create FFTW plan for 4D double-precision backward transform\n");
    backward_plan = fftwf_plan_dft(4, N, x, x, FFTW_BACKWARD, FFTW_ESTIMATE);
    if (0 == backward_plan) goto failed;

    printf("Initialize input for forward transform\n");
    init(x, N[0],N[1],N[2],N[3], H[0],H[1],H[2],H[3]);

    printf("Compute forward FFT\n");
    fftwf_execute(forward_plan);

    printf("Verify the result of forward FFT\n");
    status = verify(x, N[0],N[1],N[2],N[3], H[0],H[1],H[2],H[3]);
                 ...
                 int H1,int H2,int H3,int H4)
{
    ...
    int S1 = N2*N3*N4, S2 = N3*N4, S3 = N4, S4 = 1;
        ...
        for (n2 = 0; n2 < N2; n2++)
        {
            for (n3 = 0; n3 < N3; n3++)
            {
                for (n4 = 0; n4 < N4; n4++)
                {
                    ...
                    phase += moda(n4,H4,N4) / N4;
                    index = n1*S1 + n2*S2 + n3*S3 + n4*S4;
                    x[index][0] = cosf( TWOPI * phase ) / (N1*N2*N3*N4);
                    x[index][1] = sinf( TWOPI * phase ) / (N1*N2*N3*N4);
                }
            }
        }
    }
                  ...
                  int H1, int H2, int H3, int H4)
{
     ...
     * input data
     */
    errthr = 5.0f * logf( (float)N1*N2*N3*N4 ) / logf(2.0f) * FLT_EPSILON;
