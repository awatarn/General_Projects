...
/*
! Content:
!       Example of using fftw_plan_guru_dft_c2r function.
!
!****************************************************************************/
...
int main(void)
{
    ...
    printf("Example dp_plan_guru_dft_c2r\n");
    ...
    printf("Create FFTW plan for c2r transform\n");
    {
        ...
        c2r = fftw_plan_guru_dft_c2r(3, dim, 1, &M_dim, x, (double*)x, FFTW_ESTIMATE);
    ...
    printf("Create FFTW plan for r2c transform\n");
    {
        ...
        r2c = fftw_plan_guru_dft_r2c(3, dim, 1, &M_dim, (double*)x, x, FFTW_ESTIMATE);
    ...
    status = verify_r((double*)x, N, rstride, M, rdist, H);
    if (0 != status) goto failed;

    printf("Initialize input for r2c FFT\n");
    init_r((double*)x, N, rstride, M, rdist, H);

    printf("Compute r2c FFT using new-array function\n");
    fftw_execute_dft_r2c(r2c, (double*)x, x);
