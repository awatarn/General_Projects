...
/*
! Content:
!       Example of using fftwf_plan_guru64_dft_r2c function.
!
!****************************************************************************/
...
static void init_r(float *x, ptrdiff_t *N, ptrdiff_t *rs, ptrdiff_t M,
                   ptrdiff_t rd, ptrdiff_t *H);
static void init_c(fftwf_complex *x, ptrdiff_t *N, ptrdiff_t *rs,ptrdiff_t M,
                   ptrdiff_t rd, ptrdiff_t *H);
static int verify_r(float *x,  ptrdiff_t *N, ptrdiff_t *rs, ptrdiff_t M,
                    ptrdiff_t rd, ptrdiff_t *H);
static int verify_c(fftwf_complex *x,  ptrdiff_t *N, ptrdiff_t *rs, ptrdiff_t M,
                    ptrdiff_t rd, ptrdiff_t *H);
...
int main(void)
{
    ...
    printf("Example sp_plan_guru64_dft_r2c\n");
    ...
    printf("Define layout of data in real domain\n");
    /*
     * Model declaration x[ M ][ N[0] ][ N[1] ][ 2*(N[2]/2+1) ].
     */
    cstride[2] = 1;
    ...
    x  = fftwf_malloc(sizeof(float) * nr );
    if (0 == x) goto failed;
    ...
    printf("Create FFTW plan for r2c transform\n");
    {
        ...
        r2c = fftwf_plan_guru64_dft_r2c(3, dim, 1, &M_dim, x, (fftwf_complex*)x,
		                               FFTW_ESTIMATE);
    ...
    printf("Create FFTW plan for c2r transform\n");
    {
        ...
        c2r = fftwf_plan_guru64_dft_c2r(3, dim, 1, &M_dim, (fftwf_complex*)x, x,
		                               FFTW_ESTIMATE);
    ...
    status = verify_c((fftwf_complex*)x, N, cstride, M, cdist, H);
    if (0 != status) goto failed;

    printf("Initialize input for c2r FFT\n");
    init_c((fftwf_complex*)x, N, cstride, M, cdist, H);

    printf("Compute c2r FFT using new-array function\n");
    fftwf_execute_dft_c2r(c2r, (fftwf_complex*)x, x);
...
static void init_c(fftwf_complex *x, ptrdiff_t *N, ptrdiff_t *cs, ptrdiff_t M,
...
static int verify_c(fftwf_complex *x, ptrdiff_t *N, ptrdiff_t *cs, ptrdiff_t M,
                    ptrdiff_t cd, ptrdiff_t *H)
{
...
/* Verify that x has unit peak at H */
static int verify_r(float *x, ptrdiff_t *N, ptrdiff_t *rs, ptrdiff_t M, ptrdiff_t rd,
                    ptrdiff_t *H)
{
    float err, errthr, maxerr;
    ptrdiff_t n1, n2, n3, m, S1, S2, S3, index;
