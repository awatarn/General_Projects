...
/*
! Content:
!       Example of using fftwf_plan_dft_r2c_3d function.
!
!****************************************************************************/
...
int main(void)
{
    ...
    printf("Example sp_plan_dft_r2c_3d\n");
    ...
    x  = fftwf_malloc(sizeof(float)*2*N[0]*N[1]*(N[2]/2+1));
    if (0 == x) goto failed;

    printf("Create FFTW plan for 3D r2c inplace FFT\n");
    r2c = fftwf_plan_dft_r2c_3d(N[0], N[1], N[2], x, (fftwf_complex*)x, FFTW_ESTIMATE);
