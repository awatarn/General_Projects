...
/*
! Content:
!       Example of using fftw_plan_dft_r2c function.
!
!****************************************************************************/
...
int main(void)
{
    ...
    printf("Example dp_plan_dft_r2c\n");
    ...
    x  = fftw_malloc(sizeof(double)*N[0]*N[1]*N[2]*(N[3]/2+1)*2);
    if (0 == x) goto failed;

    printf("Create FFTW plan for 4D r2c inplace FFT\n");
    r2c = fftw_plan_dft_r2c(4, N, x, (fftw_complex*)x, FFTW_ESTIMATE);
        ...
        for (n2 = 0; n2 < N[1]; n2++)
        {
            for (n3 = 0; n3 < N[2]; n3++)
            {
                for (n4 = 0; n4 < N[3]; n4++)
                {
                    ...
                    x[index] = factor * cos( TWOPI * phase ) / (N[0]*N[1]*N[2]*N[3]);
                }
            }
        }
    }
