...
/*
! Content:
!       Example of using fftw_plan_dft_c2r_3d function.
!
!****************************************************************************/
...
int main(void)
{
    ...
    printf("Example dp_plan_dft_c2r_3d\n");
    ...
    x  = fftw_malloc(sizeof(fftw_complex)*N[0]*N[1]*(N[2]/2+1));
    printf("Allocate real data array y(%i)\n", N[0]*N[1]*N[2]);
    y  = fftw_malloc(sizeof(double)*N[0]*N[1]*N[2]);
    if (0 == x || 0 == y) goto failed;

    printf("Create FFTW plan for 3D c2r FFT\n");
    c2r = fftw_plan_dft_c2r_3d(N[0], N[1], N[2], x, y, FFTW_ESTIMATE);
