...
/*
! Content:
!       Example of using fftw_plan_dft function.
!
!****************************************************************************/
...
int main(void)
{
    ...
    x  = fftw_malloc(sizeof(fftw_complex)*N[0]*N[1]*N[2]*N[3]);
    if (0 == x) goto failed;

    printf("Create FFTW plan for 4D double-precision forward transform\n");
    forward_plan = fftw_plan_dft(4, N, x, x, FFTW_FORWARD, FFTW_ESTIMATE);
    if (0 == forward_plan) goto failed;

    printf("Create FFTW plan for 4D double-precision backward transform\n");
    backward_plan = fftw_plan_dft(4, N, x, x, FFTW_BACKWARD, FFTW_ESTIMATE);
        ...
        for (n2 = 0; n2 < N2; n2++)
        {
            for (n3 = 0; n3 < N3; n3++)
            {
                for (n4 = 0; n4 < N4; n4++)
                {
                    ...
                    x[index][0] = cos( TWOPI * phase ) / (N1*N2*N3*N4);
                    x[index][1] = sin( TWOPI * phase ) / (N1*N2*N3*N4);
                }
            }
        }
    }
                  ...
                  int H1, int H2, int H3, int H4)
{
     ...
     * input data
     */
    errthr = 5.0 * log( (double)N1*N2*N3*N4 ) / log(2.0) * DBL_EPSILON;
