...
!           preconditioner but with user-defined stopping criteria
!
!*******************************************************************************/
...
/*  Simplest case: no preconditioning and use the user-defined stopping tests. */
/*-----------------------------------------------------------------------------*/
int main (void)
{
  /*---------------------------------------------------------------------------*/
  ...
  double euclidean_norm, dpar[128 + 2 * 2], tmp[8 * (3 + 2)], temp[8];
  char tr = 'u';
  double eone = -1.E0;
  MKL_INT ione = 1;
...
rci:dcgmrhs (&n, solution, &nRhs, rhs, &rci_request, ipar, dpar, tmp);
  ...
  /* it is less than 1.E-4                                                     */
  /*---------------------------------------------------------------------------*/
  if (rci_request == 2)
    {
      mkl_dcsrsymv (&tr, &n, a, ia, ja, &solution[(ipar[2] - 1) * n], temp);
      daxpy (&n, &eone, &rhs[(ipar[2] - 1) * n], &ione, temp, &ione);
