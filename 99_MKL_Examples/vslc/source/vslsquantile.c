...
int main()
{
    ...
    float xT[N][DIM];      /* transposed matrix of observations */
    float o_stat[DIM][N];  /* matrix to store order statistics */
    float o_quant[M];
    float quantiles[DIM][M];
    ...
    errcode = vslsSSEditQuantiles( task, &m, o_quant,
                                   (float*)quantiles, (float*)o_stat,
                                   &o_storage );
    CheckVslError(errcode);
