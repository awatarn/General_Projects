...
/*
!  Content:
!  Example of 2-dimension linear correlation operation on double precision data.
!*******************************************************************************/
...
int main()
{
    ...
    int mode = VSL_CORR_MODE_DIRECT;

    /*
    *  Create task descriptor (create descriptor of problem)
    */
    status = vsldCorrNewTask(&task,mode,rank,xshape,yshape,zshape);
    ...
    */
    status = vsldCorrExec(task,x,NULL,y,NULL,z,NULL);
    if( status != VSL_STATUS_OK ){
